describe('template spec', () => {
  ...
  it('add-station-successful', function() {
    ...
    cy.wrap($row).find('td').eq(1).invoke('text').then(city => {
      if (city.trim() === 'Aveiro') {
        // Capacity is in the 3rd <td> (index 2)
        cy.wrap($row).find('td').eq(2).invoke('text').then(capacityText => {
          ...
          cy.wrap($row).find('td').eq(3).invoke('text').then(locationText => {
            const [latStr, longStr] = locationText.split(',').map(s => s.trim());
  ...
  it('edit-station-successful', function() {
    ...
    cy.get('table tbody tr').each(($row) => {
      cy.wrap($row).find('td').eq(1).invoke('text').then(cityText => {
